...
DEFAULT_DEFMAPS = [
    "/Users/macbook/Downloads/Bachelorarbeit/project/defmap_output_div_8/defmap_stack_Z1_div.npy",
    "/Users/macbook/Downloads/Bachelorarbeit/project/defmap_output_div_8/defmap_stack_Z2_div.npy",
    "/Users/macbook/Downloads/Bachelorarbeit/project/defmap_output_div_8/defmap_stack_Z3_div.npy"
]
DEFAULT_LABEL_DIR = "/Users/macbook/Downloads/Bachelorarbeit/CodeExampleRayan/yolov5/runs/detect/ch3_82/labels"
DEFAULT_OUT_DIR = "/Users/macbook/Downloads/Bachelorarbeit/project/lagcorr_new_div_8"

parser = argparse.ArgumentParser(
    description="Lag‑Korrelation YOLO‑Confidence ↔ Magnitude")
parser.add_argument("--defmap", nargs="+", default=DEFAULT_DEFMAPS,
                    help="Eine oder mehrere *.npy-Dateien mit Divergenz-Stacks (Default: DEFAULT_DEFMAPS)")
...
def run_analysis(defmap_path):
    ...
    USE_ABS      = False      # True → |diverg| ; False → signed value
    ...
    for scale, sub_df in tbl.groupby("scale"):
        ...
        ax_curve.errorbar(avg_curve.index,
                          ...
                          label=f"ROI {scale}×",
                          capsize=3)
    ax_curve.legend(loc="upper left", frameon=True, fancybox=True, edgecolor='gray')
    ...
    curve_path = os.path.join(OUT_DIR, f"lag_{'div' if 'div' in defmap_path else 'mag'}_curve_{z_tag}_{filter_tag}.png")
    ...
    avg_all = tbl.groupby("lag")["diverg"].mean()
    sem_all = tbl.groupby("lag")["diverg"].sem()
    ax_avg.errorbar(avg_all.index,
                    avg_all.values,
                    yerr=sem_all.values,
                    fmt='-o',
                    color='black',
                    capsize=3)
    ax_avg.legend(["ROI gemittelt"], loc="upper left", frameon=True, fancybox=True, edgecolor='gray')
    metric_label = "Divergenz" if "div" in defmap_path else "Magnitude"
    ax_avg.set_title(f"Gemittelte {metric_label} über Lag (alle ROIs, {z_tag})")
    ...
    avg_path = os.path.join(OUT_DIR, f"lag_{'div' if 'div' in defmap_path else 'mag'}_curve_avg_{z_tag}_{filter_tag}.png")
    fig_avg.savefig(avg_path, dpi=300)
    plt.close(fig_avg)
    print("Durchschnitt über alle ROIs gespeichert:", avg_path)
    ...
    ax_sig.set_title(f"Conf ↔ {metric_label} – signifikante Lags (★ p<0.05)")
    ax_sig.axhline(0, lw=1, color="k")
    fig_sig.tight_layout(pad=1.0)
    sig_png = os.path.join(OUT_DIR, f"lag_corr_significant_{'div' if 'div' in defmap_path else 'mag'}_{z_tag}_{filter_tag}.png")
